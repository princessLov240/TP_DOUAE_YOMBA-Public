...
void handle_redirections(char *argv[]) {
    for (int i = 0; argv[i] != NULL; i++) {
        ...
        if (strcmp(argv[i], "<") == 0) {
            ...
            dup2(fd, STDIN_FILENO); // change stdin to the file
            close(fd);
            
            for (int j = i; argv[j] != NULL; j++) {
            argv[j] = argv[j + 2]; // Shift arguments left
        }
        ...
        if (strcmp(argv[i], ">") == 0) {
            int fd = open(argv[i + 1],
                          ...
                          0644);
            if (fd < 0) {
                perror("fail to open file for output redirection");
                exit(EXIT_FAILURE);
            }
            dup2(fd, STDOUT_FILENO);// change stdout to the file
            close(fd);
...
/* Execute a command )
Redirection to stdin or stdout with < or > is handled here */
void execute_command(const char *command, int *exit_code, int *signal_num, long *duration_ms) {
    ...
    if (pid < 0) {
        ...
        handle_redirections(args); // Handle any redirections
        execvp(args[0], args);
